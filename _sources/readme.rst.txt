...
主要是跟大家一起探讨和学习人工智能、深度学习的计算机系统设计，而整个系统是围绕着
...
+====+====================================+============================+
| 1  | `AI框架基                          | AI                         |
|    | 础 <./Frontend/01_Foundation/>`__  | 框架的作用、发展、编程范式 |
+----+------------------------------------+----------------------------+
| 2  | `自动                              | 自动微分的实现方式和原理   |
|    | 微分 <./Frontend/02_AutoDiff/>`__  |                            |
+----+------------------------------------+----------------------------+
| 3  | `计                                | 计算图的概念，             |
|    | 算图 <./Frontend/03_DataFlow/>`__  | 图优化、图执行、控制流表达 |
+----+------------------------------------+----------------------------+
| 4  | `分布式                            | AI集群                     |
|    | 集群 <./Frontend/04_AICluster>`__  | 服务器架构、软硬件通信方式 |
+----+------------------------------------+----------------------------+
| 5  | `分                                | 大模型挑战，Tr             |
|    | 布式算法 <./Frontend/05_AIAlgo>`__ | ansformer和MOE结构的大模型 |
+----+------------------------------------+----------------------------+
| 6  | `分布                              | 数据并行、模型             |
|    | 式并行 <./Frontend/06_Parallel>`__ | 并行、混合并行的原理和策略 |
+----+------------------------------------+----------------------------+
|    |                                    |                            |
+----+------------------------------------+----------------------------+

`三. AI编译原理 <./Compiler/>`__
...
+------+---------------------------------+-----------------------------+
| 1    | `传统编译                       | 传统编                      |
|      | 器 <./Compiler/01_Tradition>`__ | 译器GCC与LLVM，LLVM详细架构 |
+------+---------------------------------+-----------------------------+
| 2    | `AI                             | AI编译器发展                |
|      | 编译器                          | 与架构定义，未来挑战与思考  |
|      |  <./Compiler/02_AICompiler>`__  |                             |
+------+---------------------------------+-----------------------------+
| 3    | `前端优                         | AI编译器的前端              |
|      | 化 <./Compiler/03_Frontend>`__  | 优化(算子融合、内存优化等)  |
+------+---------------------------------+-----------------------------+
| 4    | `后端                           | AI编译器的后端优            |
|      | 优化 <./Compiler/04_Backend>`__ | 化(Kernel优化、AutoTuning)  |
+------+---------------------------------+-----------------------------+
| 5    | 多面体                          | 待更ing…                    |
+------+---------------------------------+-----------------------------+
| 6    | `PyTorc                         | PyTorch2.                   |
|      | h2.0 <./Compiler/06_PyTorch>`__ | 0最重要的新特性：编译技术栈 |
+------+---------------------------------+-----------------------------+
|      |                                 |                             |
+------+---------------------------------+-----------------------------+

`四. AI推理系统 <./Inference/>`__
...
+-------+-----------------------------------+--------------------------+
| 1     | `推理系                           | 推理系统整               |
|       | 统 <./Inference/01_Inference/>`__ | 体介绍，推理引擎架构梳理 |
+-------+-----------------------------------+--------------------------+
| 2     | `轻量网                           | 轻量化主干网络，         |
|       | 络 <./Inference/02_Mobilenet/>`__ | MobileNet等SOTA模型介绍  |
+-------+-----------------------------------+--------------------------+
| 3     | `模                               | 模型压缩4件套，          |
|       | 型压缩 <./Inference/03_Slim/>`__  | 量化、蒸馏、剪枝和二值化 |
+-------+-----------------------------------+--------------------------+
| 4     | `转换&优                          | AI框架训练后模型         |
|       | 化 <./Inference/04_Converter/>`__ | 进行转换，并对计算图优化 |
+-------+-----------------------------------+--------------------------+
| 5     | `Kerne                            | Kernel层、算子层优化     |
|       | l优化 <./Inference/05_Kernel/>`__ | ，对算子、内存、调度优化 |
+-------+-----------------------------------+--------------------------+

`五. AI芯片架构 <./Hardware/>`__
...
| 1    | `AI                               | 神经网络等AI技术          |
|      | 计算体                            | 的计算模式和计算体系架构  |
|      | 系 <./Hardware/01_Foundation/>`__ |                           |
+------+-----------------------------------+---------------------------+
| 2    | `AI                               | CPU、GPU、                |
|      | 芯片                              | NPU等芯片体系架构基础原理 |
|      | 基础 <./Hardware/02_ChipBase/>`__ |                           |
+------+-----------------------------------+---------------------------+
| 3    | `图形处理器                       | GPU的基本                 |
|      | GPU <./Hardware/03_GPUBase/>`__   | 原理，英伟达GPU的架构发展 |
+------+-----------------------------------+---------------------------+
| 4    | `英伟达GPU详                      | 英伟达GPU的T              |
|      | 解 <./Hardware/04_GPUDetail/>`__  | ensorCore、NVLink深度剖析 |
+------+-----------------------------------+---------------------------+
| 5    | `AI                               | 华为、谷歌、特斯          |
|      | 专用处理器 <./Hardware/05_NPU>`__ | 拉等专用AI处理器核心原理  |
+------+-----------------------------------+---------------------------+
